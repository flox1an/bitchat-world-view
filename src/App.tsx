...
function ChatLine({
  ...
  const pubkeyPostfix = ev.pubkey.slice(-4);
  ...
  return (
    <div className="whitespace-pre-wrap break-all mb-1">
      ...
      <span className="text-cyan-300">
        &lt;{nickname}
        <span className="text-gray-600">#{pubkeyPostfix}</span>&gt;
      </span>{" "}
      <span className="text-gray-100">{renderTextWithLinks(msg)}</span>{" "}
      {showGeohash && geohash && (
        <span className="text-fuchsia-300">#{geohash}</span>
      )}
    </div>
  );
}
...
export default function App() {
  ...
  const events = useObservableMemo(
    () =>
      pool.subscription(RELAYS, { kinds: [20000], limit: 500 }).pipe(
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter(
      (event: NostrEvent) => getTagValue(event, "g") === selectedChatroom
    );
            ...
            <div className="p-2">
              {chatrooms.map(room => (
                <button
                  ...
                  onClick={() =>
                    setSelectedChatroom(
                      selectedChatroom === room.geohash ? null : room.geohash
                    )
                  }
        ...
        >
          {!filteredEvents || filteredEvents.length === 0 ? (
            <div className="text-gray-500">
              {selectedChatroom
                ? `No messages in #${selectedChatroom} yet...`
                : "Waiting for ephemeral events…"}
            </div>
          ) : (
